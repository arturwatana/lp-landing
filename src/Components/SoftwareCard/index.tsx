...
export default function SoftwareCard({index, text, isInView}: SoftwareCardProps){
    ...
    return (
        <>
            <motion.div style={{width:"80%", gap:"20px", display: "flex", position:"relative"}}>
                <motion.div initial={{opacity: 0, y: -50}}  variants={variants}  custom={index} onHoverStart={() => setIsHovered(true)} onHoverEnd={() => setIsHovered(false)}  animate={isInView ? "translate" : ""} whileHover={{scale: 1.1}} style={{cursor: "pointer", maxWidth: "200px", width: "200px", justifyContent:"space-around", display:"flex", minHeight: "50px" ,alignItems:"center", gap:"10px", boxShadow:"-4px 9px 19px 0px rgba(0, 0, 0, 0.05)", padding: "3px 6px", borderRadius: "10px"}}>
                ...
                <motion.div style={{background:"#702CF299", color:"white", border:"1px solid gray", left:"25%", position:"absolute", width: "40%", height: "100px", borderRadius:"10px", padding: "20px"}} variants={variants} initial={{display:"none", opacity: 0}} animate={isHovered ? "show" : ""} >
